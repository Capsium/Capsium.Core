...
using Capsium.Units;
using System;
using static Capsium.Core.Interop;

namespace Capsium.Devices
{
    public partial class F7GPIOManager : ICapsiumIOController
    {
        ...
        public void ConfigureAnalogInput(IPin pin)
        {
            ...
            // on the Capsium, all ADCs are in in ADC1
        ...
        private bool InitializeADC()
        {
            ...
            // enable the ADC1 clock - all Capsium ADCs are in ADC1
            ...
            UPD.UpdateRegister(STM32.Capsium_ADC1_BASE + STM32.ADC_SR_OFFSET,
                0x1f, 0);
            ...
            UPD.UpdateRegister(STM32.Capsium_ADC1_BASE + STM32.ADC_CR1_OFFSET,
            ...
            // 
            UPD.UpdateRegister(STM32.Capsium_ADC1_BASE + STM32.ADC_CR2_OFFSET,
            ...
            // 
            UPD.UpdateRegister(STM32.Capsium_ADC1_BASE + STM32.ADC_SMPR1_OFFSET,
            ...
            // 
            UPD.UpdateRegister(STM32.Capsium_ADC1_BASE + STM32.ADC_SMPR2_OFFSET,
            ...
            //  One (1) conversion 
            // 
            UPD.UpdateRegister(STM32.Capsium_ADC1_BASE + STM32.ADC_SQR1_OFFSET,
                STM32.ADC_SQR1_NON_RESERVED_MASK, 0);

            // Set up the SQR2 sequence register.  This translates to:
            //  no conversions 7-12 
            // 
            UPD.UpdateRegister(STM32.Capsium_ADC1_BASE + STM32.ADC_SQR2_OFFSET,
            ...
            //  no conversions 0-6 
            // 
            UPD.UpdateRegister(STM32.Capsium_ADC1_BASE + STM32.ADC_SQR3_OFFSET,
            ...
            //  independent ADCs
            // 
            UPD.UpdateRegister(STM32.Capsium_ADC1_BASE + STM32.ADC_CCR_OFFSET,
        ...
        public Temperature GetTemperature()
        {
            ...
            // read the CCR
            var ccr = UPD.GetRegister(STM32.Capsium_ADC1_BASE + STM32.ADC_CCR_OFFSET);

            // disable the VBAT and enable the temp sensor
            var tempCCR = (int)ccr;
            tempCCR &= ~(1 << STM32.ADC_CCR_VBATE_SHIFT);
            tempCCR |= 1 << STM32.ADC_CCR_TSVREFE_SHIFT;
            UPD.SetRegister(STM32.Capsium_ADC1_BASE + STM32.ADC_CCR_OFFSET, (uint)tempCCR);

            // read channel 18
            var adc = GetAnalogValue(18, 500);

            // restore the CCR
            UPD.SetRegister(STM32.Capsium_ADC1_BASE + STM32.ADC_CCR_OFFSET, ccr);
        ...
        private int GetAnalogValue(int channel, int timeout = DefaultAdcTimeoutMs)
        {
            ...
            do
            {
                var register_sr = UPD.GetRegister(STM32.Capsium_ADC1_BASE + STM32.ADC_SR_OFFSET);
            ...
            return (int)UPD.GetRegister(STM32.Capsium_ADC1_BASE + STM32.ADC_DR_OFFSET);
        }
    }
}
