...
internal unsafe class F7CellNetworkAdapter : NetworkAdapterBase, ICellNetworkAdapter
{
    ...
    private void UpdateAtCmdsOutput()
    {
        ...
        try
        {
            var len = Core.Interop.Nuttx.Capsium_get_cell_at_cmds_output(buffer);
    ...
    public override bool IsConnected
    {
        get
        {
            try
            {
                return Core.Interop.Nuttx.Capsium_cell_is_connected();
            }
            catch (Exception e)
            {
                Resolver.Log.Error($"Failed to read Capsium_cell_is_connected(): {e.Message}");
                return false;
            }
        }
    }
    ...
    {
        return Core.Interop.Nuttx.CapsiumCellNetworkScanner();
    }
