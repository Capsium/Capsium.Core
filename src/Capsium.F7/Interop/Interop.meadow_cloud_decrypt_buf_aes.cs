...
internal static partial class Interop
{
    public static partial class Nuttx
    {
        ...
        private static extern int Capsium_cloud_decrypt_buf_aes(IntPtr encrypted_buf, int encrypted_len, IntPtr key, IntPtr iv, IntPtr decrypted_buf);

        const int aes_key_size = 32;

        const int aes_iv_size = 16;

        public static byte[] CapsiumCloudDecryptAES(byte[] encrypted_data, byte[] key, byte[] iv)
        {
            ...
            try
            {
                ...
                var decryptLength = Capsium_cloud_decrypt_buf_aes(encryptedBufferPtr, encrypted_data.Length, keyBufferPtr, ivBufferPtr, decryptedBufferPtr);
