...
public partial class SimulatedCapsium<TPinDefinitions> : ISimulatedDevice<TPinDefinitions>
    where TPinDefinitions : IPinDefinitions, new()
{
    ...
    public IAnalogInputPort CreateAnalogInputPort(IPin pin, int sampleCount, TimeSpan sampleInterval, Capsium.Units.Voltage voltageReference)
    {
    ...
    public IBiDirectionalPort CreateBiDirectionalPort(IPin pin, bool initialState = false, ResistorMode resistorMode = ResistorMode.Disabled, PortDirectionType initialDirection = PortDirectionType.Input)
    {
        return CreateBiDirectionalPort(pin, initialState, resistorMode, initialDirection, TimeSpan.Zero, TimeSpan.Zero);
    }
    ...
    public ISpiBus CreateSpiBus(IPin clock, IPin mosi, IPin miso, Capsium.Units.Frequency speed)
    {
        throw new NotImplementedException();
    }
