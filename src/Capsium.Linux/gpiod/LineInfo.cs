...
using static Capsium.Gpiod.Interop;
...
namespace Capsium
{
    ...
    internal class LineInfo
    {
        ...
        private const string CapsiumConsumer = "Capsium";
        ...
        public void Request(line_direction direction)
        {
            ...
            {
                result = gpiod_line_request_input(Handle, CapsiumConsumer);
            }
            else
            {
                result = gpiod_line_request_output(Handle, CapsiumConsumer);
            }
        ...
        {
            // TODO: check for free?
            var result = gpiod_line_request_input_flags(Handle, CapsiumConsumer, flags);
        ...
        public void RequestInterrupts(InterruptMode mode, line_request_flags flags)
        {
            ...
            switch (mode)
            {
                case InterruptMode.EdgeRising:
                    result = gpiod_line_request_rising_edge_events_flags(Handle, CapsiumConsumer, flags);
                    break;
                case InterruptMode.EdgeFalling:
                    result = gpiod_line_request_falling_edge_events(Handle, CapsiumConsumer);
                    break;
                case InterruptMode.EdgeBoth:
                    result = gpiod_line_request_both_edges_events(Handle, CapsiumConsumer);
                    break;
                default:
                    return;
            }
